        ...
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
          <div class="flex items-center justify-between px-6 py-4">
            <div class="flex items-center space-x-4">
              ...
              <div>
                <h1 class="text-2xl font-bold text-gray-800">Laporan Rawat Inap</h1>
        ...
        <main class="p-6">
          <div class="mb-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div class="mb-6">
                <h2 class="text-3xl font-bold text-gray-800 mb-2">Buat Laporan Rawat Inap</h2>
                <p class="text-gray-600">Pilih filter tanggal masuk dan upload file data.</p>
              </div>
  
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                   <div>
                      <label for="startDate" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Masuk (Mulai)</label>
                  ...
                  <div>
                      <label for="endDate" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Masuk (Selesai)</label>
                ...
                <div>
                    <label for="ward" class="block text-sm font-medium text-gray-700 mb-2">Ruang Perawatan</label>
                    <select id="ward" v-model="selectedWard" class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:ring-[#5E936C] focus:border-[#5E936C] transition duration-200">
                      <option value="semua">Semua Ruangan</option>
                      <option value="mawar">Ruang Mawar</option>
                      <option value="melati">Ruang Melati</option>
                      <option value="tulip">Ruang Tulip</option>
                  ...
                  <div>
                    <label for="paymentType" class="block text-sm font-medium text-gray-700 mb-2">Jenis Pembayaran</label>
                    <select id="paymentType" v-model="paymentType" class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:ring-[#5E936C] focus:border-[#5E936C] transition duration-200">
              ...
              <div class="mb-6">
                  <label class="block text-sm font-medium text-gray-700 mb-2">File Data Rawat Inap</label>
          ...
          <div v-if="reportData" class="animate-fadeIn space-y-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
              <div class="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-200">
                  <div>
                      <h3 class="text-lg font-bold text-gray-800">Detail Data Pasien Rawat Inap</h3>
                      <p class="text-sm text-gray-600 mt-1">Menampilkan {{ reportData.details.length }} data pasien.</p>
                  </div>
                  ...
                  <table class="w-full text-sm text-left text-gray-600">
                      <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                          <tr>
                              ...
                              <th scope="col" class="px-6 py-3">Tgl Keluar</th>
                              <th scope="col" class="px-6 py-3">No. RM</th>
                              <th scope="col" class="px-6 py-3">Nama Pasien</th>
                              <th scope="col" class="px-6 py-3">Ruang Rawat</th>
                              <th scope="col" class="px-6 py-3">Diagnosis Awal</th>
                              <th scope="col" class="px-6 py-3">DPJP</th>
                          </tr>
                      </thead>
                      <tbody>
                          <tr v-for="(item, index) in reportData.details" :key="index" class="bg-white border-b hover:bg-gray-50">
                              <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ item.tglMasuk }}</td>
                              <td class="px-6 py-4 whitespace-nowrap">{{ item.tglKeluar }}</td>
                              <td class="px-6 py-4">{{ item.norm }}</td>
                              <td class="px-6 py-4">{{ item.pasien }}</td>
                              <td class="px-6 py-4">{{ item.ruang }}</td>
                              <td class="px-6 py-4">{{ item.diagnosis }}</td>
                              <td class="px-6 py-4">{{ item.dpjp }}</td>
                          </tr>
                      </tbody>
                  </table>
              </div>
    ...
    methods: {
      ...
      async handleProses() {
        ...
        this.reportData = {
          ...
          summaryStats: [
            { label: 'Total Pasien Masuk', value: 315, icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" />', bgColor: 'bg-blue-100', iconColor: 'text-blue-600' },
            { label: 'Avg. LOS', value: '4.2 Hari', icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />', bgColor: 'bg-green-100', iconColor: 'text-green-600' },
            { label: 'BOR', value: '85.6%', icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7.C14 5 16.09 5.777 17.656 7.343A8 8 0 0117.657 18.657z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.879 16.121A3 3 0 1014.12 11.88l-4.242 4.242z" />', bgColor: 'bg-yellow-100', iconColor: 'text-yellow-600' },
            { label: 'Pasien Pulang', value: '298', icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16l-4-4m0 0l4-4m-4 4h14m-5-4v1a3 3 0 003 3h6a3 3 0 003-3V7a3 3 0 00-3-3h-7a3 3 0 00-3 3v1" />', bgColor: 'bg-indigo-100', iconColor: 'text-indigo-600' }
          ],
          details: [
              { tglMasuk: '2025-09-11', tglKeluar: '2025-09-15', norm: '223344', pasien: 'Bambang Pamungkas', ruang: 'Ruang Mawar', diagnosis: 'Dengue Fever', dpjp: 'Dr. Santoso' },
              { tglMasuk: '2025-09-11', tglKeluar: '2025-09-14', norm: '223345', pasien: 'Rina Herawati', ruang: 'Ruang Melati', diagnosis: 'Pneumonia', dpjp: 'Dr. Anisa' },
              { tglMasuk: '2025-09-10', tglKeluar: '2025-09-20', norm: '223346', pasien: 'Cahyo Utomo', ruang: 'ICU', diagnosis: 'Stroke', dpjp: 'Dr. Gunawan' },
              { tglMasuk: '2025-09-09', tglKeluar: '2025-09-12', norm: '223347', pasien: 'Dian Puspita', ruang: 'Ruang Tulip', diagnosis: 'Post Partum', dpjp: 'Dr. Ratna' },
              { tglMasuk: '2025-09-08', tglKeluar: '2025-09-13', norm: '223348', pasien: 'Endang Lestari', ruang: 'Ruang Mawar', diagnosis: 'Typhoid Fever', dpjp: 'Dr. Santoso' },
          ]
        }
      ...
      handleCancel() {
        ...
        this.reportData = null
        this.isProcessing = false
        this.startDate = formatDate(new Date(new Date().setDate(new Date().getDate() - 7)))
        this.endDate = formatDate(new Date())
        this.selectedWard = 'semua'
        this.paymentType = 'semua'
        if(this.$refs.fileInput) {
          this.$refs.fileInput.value = ''
        }
      },
      handleDownload() {
        const filename = `Laporan_Rawat_Inap_${this.reportData.startDate}_to_${this.reportData.endDate}.xlsx`
