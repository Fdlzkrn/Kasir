        ...
        <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
          <div class="flex items-center justify-between px-6 py-4">
            <div class="flex items-center space-x-4">
              ...
              <div>
                <h1 class="text-2xl font-bold text-gray-800">Laporan IGD</h1>
        ...
        <main class="p-6">
          <div class="mb-8">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div class="mb-6">
                <h2 class="text-3xl font-bold text-gray-800 mb-2">Buat Laporan IGD</h2>
                <p class="text-gray-600">Pilih filter profesional dan upload file data IGD.</p>
              </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                ...
                <div>
                    <label for="triage" class="block text-sm font-medium text-gray-700 mb-2">Level Triage</label>
                    <select id="triage" v-model="triageLevel" class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:ring-[#5E936C] focus:border-[#5E936C] transition duration-200">
                      <option value="semua">Semua Level</option>
                      <option value="Merah">Merah (Resusitasi)</option>
                      <option value="Kuning">Kuning (Urgensi)</option>
                      <option value="Hijau">Hijau (Non-Urgensi)</option>
                      <option value="Hitam">Hitam (Meninggal)</option>
                    </select>
                  </div>
                  <div>
                    <label for="disposition" class="block text-sm font-medium text-gray-700 mb-2">Tindak Lanjut (Disposisi)</label>
                    <select id="disposition" v-model="disposition" class="w-full px-4 py-3 border border-gray-300 rounded-lg bg-white focus:ring-[#5E936C] focus:border-[#5E936C] transition duration-200">
              ...
              <div class="mb-6">
                  <label class="block text-sm font-medium text-gray-700 mb-2">File Data IGD</label>
          ...
          <div v-if="reportData" class="animate-fadeIn space-y-8">
            <div>
              <h3 class="text-xl font-bold text-gray-800 mb-4">Ringkasan Kinerja IGD ({{ reportData.startDate }} - {{ reportData.endDate }})</h3>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
              <div class="p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-gray-200">
                  <div>
                      <h3 class="text-lg font-bold text-gray-800">Detail Kunjungan IGD</h3>
                      <p class="text-sm text-gray-600 mt-1">Menampilkan {{ reportData.details.length }} data kunjungan.</p>
                  </div>
              ...
              <div class="overflow-x-auto">
                  <table class="w-full text-sm text-left text-gray-600">
                      <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                          <tr>
                              <th scope="col" class="px-6 py-3">Waktu Datang</th>
                              <th scope="col" class="px-6 py-3">No. RM</th>
                              <th scope="col" class="px-6 py-3">Nama Pasien</th>
                              <th scope="col" class="px-6 py-3">Triage</th>
                              <th scope="col" class="px-6 py-3">Keluhan Utama</th>
                              <th scope="col" class="px-6 py-3">Door-to-Doctor (m)</th>
                              <th scope="col" class="px-6 py-3">Tindak Lanjut</th>
                          </tr>
                      </thead>
                      <tbody>
                          <tr v-for="(item, index) in reportData.details" :key="index" class="bg-white border-b hover:bg-gray-50">
                              <td class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{{ item.waktuDatang }}</td>
                              <td class="px-6 py-4">{{ item.norm }}</td>
                              <td class="px-6 py-4">{{ item.pasien }}</td>
                              <td class="px-6 py-4">
                                  <span :class="['px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider', getTriageBadge(item.triage)]">
                                      {{ item.triage }}
                                  </span>
                              </td>
                              <td class="px-6 py-4">{{ item.keluhan }}</td>
                              <td class="px-6 py-4 text-center font-semibold">{{ item.dtd }}</td>
                              <td class="px-6 py-4">{{ item.tindakLanjut }}</td>
                          </tr>
                      </tbody>
                  </table>
              </div>
    ...
    methods: {
      ...
      async handleProses() {
        ...
        this.reportData = {
          ...
          summaryStats: [
            { label: 'Total Kunjungan', value: 189, icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a3.002 3.002 0 014.288 0M12 15a3 3 0 110-6 3 3 0 010 6z" />', bgColor: 'bg-blue-100', iconColor: 'text-blue-600' },
            { label: 'Avg. Door-to-Doctor', value: '7.2 Menit', icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />', bgColor: 'bg-yellow-100', iconColor: 'text-yellow-600' },
            { label: 'Admission Rate', value: '34.4%', icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />', bgColor: 'bg-green-100', iconColor: 'text-green-600' },
            { label: 'LWBS Rate', value: '1.6%', icon: '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7a4 4 0 11-8 0 4 4 0 018 0zM9 14a6 6 0 00-6 6v1h12v-1a6 6 0 00-6-6z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.718 14.828A7.942 7.942 0 0018 14a8 8 0 10-15.132 3.535" />', bgColor: 'bg-red-100', iconColor: 'text-red-600' }
          ],
          details: [
              { waktuDatang: '2025-09-11 08:15', norm: '334455', pasien: 'Farhan Jauhari', triage: 'Merah', keluhan: 'Nyeri Dada', dtd: 3, tindakLanjut: 'Rawat Inap' },
              { waktuDatang: '2025-09-11 08:22', norm: '334456', pasien: 'Gina Safitri', triage: 'Kuning', keluhan: 'Demam Tinggi', dtd: 8, tindakLanjut: 'Rawat Inap' },
              { waktuDatang: '2025-09-11 08:31', norm: '334457', pasien: 'Hadi Wibowo', triage: 'Hijau', keluhan: 'Batuk Pilek', dtd: 15, tindakLanjut: 'Pulang' },
              { waktuDatang: '2025-09-11 08:45', norm: '334458', pasien: 'Ika Nuraini', triage: 'Kuning', keluhan: 'Mual Muntah', dtd: 9, tindakLanjut: 'Pulang' },
              { waktuDatang: '2025-09-11 09:02', norm: '334459', pasien: 'Joko Santoso', triage: 'Hijau', keluhan: 'Luka Lecet', dtd: 12, tindakLanjut: 'Pulang' },
          ]
        }
      ...
      handleCancel() {
        ...
        this.triageLevel = 'semua'
        this.disposition = 'semua'
        if(this.$refs.fileInput) {
          this.$refs.fileInput.value = ''
        }
      },
      handleDownload() {
        const filename = `Laporan_IGD_${this.reportData.startDate}_to_${this.reportData.endDate}.xlsx`
